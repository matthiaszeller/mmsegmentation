...
class UPerHead3D(BaseDecodeHeadPseudo3D):
    def __init__(self, pool_scales=(1, 2, 3, 6), **kwargs):
        super().__init__(input_transform='multiple_select', slices_to_batch=False, **kwargs)
        # PSP Module
...
class UPerHeadPseudo3D(BaseDecodeHeadPseudo3D):
    ...
    def __init__(self, n_slice: int = 4, pool_scales=(1, 2, 3, 6), **kwargs):
        super().__init__(n_slice=n_slice, input_transform='multiple_select', **kwargs)
        ...
        self.bottleneck = ConvModule(
            ...
            act_cfg=self.act_cfg)
        # The head per frame
        kwargs['in_channels'] = [(i // self.n_slice) for i in self.in_channels]
        kwargs['in_channels'][-1] += self.channels_per_frame

        kwargs['channels'] = self.channels // self.n_slice

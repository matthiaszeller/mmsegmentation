...
class BlockWithRPE(nn.Module):
    ...
    def __init__(self,
        ...
        self.norm1 = build_norm_layer(norm_cfg, dim)[1] if with_attn else None
        self.attn = Attention(
            input_size,
            dim,
            num_heads=num_heads,
...
class PatchMerge(nn.Module):
    ...
    def __init__(self, dim, norm_cfg):
        super().__init__()
        self.norm = build_norm_layer(norm_cfg, dim * 4)[1]
...
class HiViT(BaseModule):
    ...
    def interpolate_pos_encoding(self, patch_pos_embed: torch.Tensor, H0, W0, H, W):
        ...
        assert H0 * W0 == N, 'original image size does not match with shape of absolute positional encoding'
        w = W // self.patch_size
        h = H // self.patch_size
    ...
    def _format_output(self, x: torch.Tensor, H0: int, W0: int) -> torch.Tensor:
        ...
        # unflatten patches, i.e. reshape to (B, Hp, Wp, H', W', C)
        x = x.reshape(B, Hp, Wp, H, W, C)
        # rearrange axes to (B, C, Hp, H', Wp, W')
        x = x.permute(0, 5, 1, 3, 2, 4)
        # flatten spatial dimensions of patches
        x = x.reshape(B, C, H * Hp, W * Wp)

        return x.contiguous()

    def forward(self, x):
        B, C, H, W = x.shape
        assert H % self.patch_size == 0 and W % self.patch_size == 0, 'input size must be divisible by the patch size'
        ...
        # at this point, patch resolution is 1x1
        # and data shape is (B, num_patches, 1, 1, C)
        x = x[..., 0, 0, :]

        if self.ape:
            # for training, no interpolation
            # for inference, interpolate to current image size
            x = x + self.interpolate_pos_encoding(self.pos_embed, *self.img_size, H, W)
        x = self.pos_drop(x)
